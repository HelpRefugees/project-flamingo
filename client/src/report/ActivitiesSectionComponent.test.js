...
describe("activities section", () => {
  ...
  describe("add a new activity", () => {
    it("adds a new empty activity subsection", () => {
      const newLength = activities.length + 1;
      wrapper
        .find(KeyActivitySubsection)
        .get(activities.length - 1)
        .props.addActivity();

      expect(wrapper.find(KeyActivitySubsection)).toHaveLength(newLength);

      expect(
        wrapper.find(KeyActivitySubsection).get(newLength - 1).props.activity
      ).toEqual({});
    });

    it("calls the onChange method with an event", () => {
      wrapper
        .find(KeyActivitySubsection)
        .get(activities.length - 1)
        .props.addActivity();
  ...
  describe("remove an activity", () => {
    it("removes the activity subsection", () => {
      const newLength = activities.length - 1;
      wrapper
        .find(KeyActivitySubsection)
        .get(0)
      ...
      expect(wrapper.find(KeyActivitySubsection).get(0).props.activity).toEqual(
        activities[1]
      );
    });
